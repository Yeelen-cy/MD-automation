...
class TaskManager:
    ...
    def _calculate_priority(self, system_dir):
        """基于体系大小计算优先级"""
        gro_file = os.path.join(system_dir, 'parameters', 'gmx.gro')
        top_file = os.path.join(system_dir, 'parameters', 'gmx.top')
    ...
    def is_tasks_empty(self):
        """检查任务队列和运行任务是否全部完成"""
        with self.lock:
            return self.task_queue.empty() and len(self.running_tasks) == 0
...
class MDExecutor:
    ...
    def __init__(self, system_dir, gpu_id):
        ...
        self.gpu_id = gpu_id
        os.makedirs(os.path.join(self.system_dir, 'pre-equ'), exist_ok=True)  # <-- 新增此行
    ...
    def _init_logger(self):
        ...
        # 文件日志
        log_file = os.path.join(self.system_dir, 'pre-equ', 'simulation.log')
    ...
    def modify_mdp(self, mdp_type, cycle=0):
        ...
        dest_dir = os.path.join(self.system_dir, 'pre-equ', f'{mdp_type}{dir_suffix}')
    ...
    def run_em(self):
        """能量最小化流程"""
        em_dir = os.path.join(self.system_dir, 'pre-equ', 'em')
        os.makedirs(em_dir, exist_ok=True)

        cmd = (f"gmx_mpi grompp -f {self.modify_mdp('em')} "  # 使用默认cycle=0
               f"-c {os.path.join(self.system_dir, 'parameters', 'gmx.gro')} "
               f"-r {os.path.join(self.system_dir, 'parameters', 'gmx.gro')} "
               f"-p {os.path.join(self.system_dir, 'parameters', 'gmx.top')} "
    ...
    def run_nvt(self, cycle):
        """NVT平衡"""
        stage_dir = os.path.join(self.system_dir, 'pre-equ', f'nvt{cycle}')
        prev_dir = os.path.join(self.system_dir, 'pre-equ', 'em') if cycle == 1 else os.path.join(self.system_dir, 'pre-equ', f'npt{cycle-1}')
    ...
    def run_npt(self, cycle):
        """NPT平衡"""
        stage_dir = os.path.join(self.system_dir, 'pre-equ', f'npt{cycle}')
        prev_dir = os.path.join(self.system_dir, 'pre-equ',f'nvt{cycle}')
...
def worker(task_manager):
    ...
    try:
        while True:
            ...
            if not system_dir:
                # 只有当没有运行中的任务且任务队列为空时才退出
                if task_manager.is_tasks_empty() and len(task_manager.running_tasks) == 0:
                    break
                time.sleep(10)
                continue
            ...
            try:
                ...
                logging.error(f"任务执行异常: {system_dir}, 错误: {str(e)}")
                # 失败任务重新加入队列
                task_manager.add_system(system_dir)
        ...
        logging.info("Worker 接收到中断信号，正在退出...")
    finally:
        logging.info("Worker 进程退出")
...
def main():
    ...
    systems = [d for d in os.listdir() 
              ...
              os.path.exists(os.path.join(d, 'parameters', 'gmx.gro')) and
    ...
    with Pool(processes=len(CONFIG['GPU_DEVICES'])) as pool:
        results = [pool.apply_async(worker, (task_manager,)) for _ in range(len(CONFIG['GPU_DEVICES']))]
        while not task_manager.is_tasks_empty():
            time.sleep(10)
        pool.terminate()
        pool.join()
    logging.info("所有任务已完成，脚本正常退出")
